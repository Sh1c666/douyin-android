...
一个**只读**的抖音 Android 客户端：刷推荐流 / 看评论 / 看主页 / 看直播，**无任何互动**。模仿 [PiliPlus](https://github.com/bggRGjQaUbCoE/PiliPlus) 的思路——**App 直接调抖音官方接口，签名在端上做，不依赖任何本地后端**。装上 APK、粘一次 cookie 就能用。

```
                    ┌──────────────────────────────┐
                    │  Android App (Kotlin/Compose) │
                    │  · 隐藏 WebView 跑 dy_ab_web.js │──┐
                    │    生成 a_bogus（端上签名）      │  │ 签名后的请求
                    │  · Retrofit 直连抖音 web API     │──┼──────────────► 抖音服务器
                    │  · cookie 存本地（粘贴一次）     │  │
                    │  · ExoPlayer 直播视频 CDN        │  │ 视频流
                    └──────────────────────────────┘◄─┘
                     不需要 PC、不需要 Python、不需要联网到家里的电脑
```
...
- **直播**：粘 `https://live.douyin.com/123456789` 或房间号 → 进入。
...
| 项 | 状况 | 应对 |
|---|---|---|
| **搜索** | 抖音对 Web 搜索单独 `verify_check` 风控，常返回空 | 客户端无搜索 UI；刷视频/评论/主页/直播不受影响 |
| **直播** | 走签名 webcast `enter` 端点取 FLV/HLS（直播页 `_ROUTER_DATA` 已不再嵌入流地址）。抓不到流时多半是**主播未开播**或抖音对该端点**风控** | 主播**正在开播**才有流；进直播间后若提示「抖音拒绝请求 status_code=…」即为风控/签名问题 |
...
| 直播黑屏 / 进不去 | 主播没在播 / 房间号不对；界面会显示具体原因（如「抖音拒绝请求 status_code=…」=风控）。深层排查 `adb logcat -s LiveFetcher` 看 enter 端点原始响应 |
